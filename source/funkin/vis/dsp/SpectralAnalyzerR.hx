...
import funkin.vis.dsp.SpectralAnalyzer.MathType;
import grig.audio.FFT;
import grig.audio.FFTVisualizationR;
import lime.media.AudioSource;

using grig.audio.lime.UInt8ArrayTools;

class SpectralAnalyzerR
{
	...
	#else
	private var fft:FFT;
	private var vis = new FFTVisualizationR();
	...
	public function new(audioSource:AudioSource, barCount:Int, smoothingTimeConstant:Float = 0.8, peakHold:Int = 30, ?manualClip:LimeAudioClip)
	{
		this.audioSource = audioSource;
		this.audioClip = manualClip != null ? manualClip : new LimeAudioClip(audioSource);
	...
	public function getLevels(?levels:Array<Bar>):Array<Bar>
	{
		...
		var bars = vis.makeLogGraph(freqs, barCount + 1, Math.floor(maxDb - minDb), 0);

		if (bars.length - 1 > barHistories.length)
		{
			barHistories.resize(cast bars.length - 1);
		}

		levels.resize(cast bars.length - 1);
		for (i in 0...bars.length - 1)
		{
			if (barHistories[i] == null)
				barHistories[i] = new RecentPeakFinder();
			var recentValues = barHistories[i];
			var value:Float = bars[i];
			...
			if (smoothingTimeConstant > 0.0 && smoothingTimeConstant < 1.0)
			{
				...
				value = smoothingTimeConstant * lastValue + (1.0 - smoothingTimeConstant) * value;
			}
	...
	public function getLevelsNormalized(?levels:Array<Bar>):Array<Bar>
	{
		if (levels == null)
			levels = new Array<Bar>();
		#if web
