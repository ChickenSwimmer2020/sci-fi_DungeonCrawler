...
class Player extends FlxSprite {
    ...
    var weaponTextTextTarget:String;
    override public function update(elapsed:Float) {
        ...
        if(canMove) velocity.x = ((velocity.x = (velocity.x.clampf(-MAX_MOVE_SPEED, MAX_MOVE_SPEED)) += (weaponKickback.x+(axis(ctrlRight, ctrlLeft) * MOVE_SPEED+(axis(ctrlSprint, [NONE])*SPRINT_MULT))))); //add kickback on-top of the normal velocity based movement
        if(canMove) velocity.y = ((velocity.y = (velocity.y.clampf(-MAX_MOVE_SPEED, MAX_MOVE_SPEED)) += (weaponKickback.y+(axis(ctrlDown, ctrlUp) * MOVE_SPEED+(axis(ctrlSprint, [NONE])*SPRINT_MULT)))));    //make sure to clamp these to the maximum move speed or else it just speeds up infinitely
